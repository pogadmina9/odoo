...
class Lead(models.Model):
    ...
    # Addon
    closing_date = fields.Datetime(string='Closing Date', tracking=True)
    presales_team = fields.Many2one('crm.presales.team', tracking=True, string='Presales')
    no_ppc = fields.Char(string='No Opty / Paket', tracking=True)
    contract_amount = fields.Monetary(string="Contract Amount", currency_field='company_currency', store=True, tracking=True)
    contact_person = fields.Many2one('res.partner', tracking=True, string='Contact Person')
    cp_phone = fields.Char(string='Phone', tracking=True)
    partner_address = fields.Char(string='Address', tracking=True)
    old_id = fields.Char(string='Old ID')
    lost_detail = fields.Html(string='Lost Detail')
    # Pipeline management
    ...
    # UX
    partner_email_update = fields.Boolean('Partner Email will Update', default=False)
    partner_phone_update = fields.Boolean('Partner Phone will Update', default=False)
    ...
    def _set_lost(self, lost_feedback):
        self.write({'lost_detail': lost_feedback})
    
    @api.onchange('contact_person')
    def _change_contact_phone_number(self):
        if self.contact_person:
            self.cp_phone = self.contact_person.mobile
        else:
            self.cp_phone = None
    
    @api.onchange('partner_id')
    def _auto_address(self):
        self.write({'partner_address': self.partner_id.contact_address_inline})
        if not self.user_id:
            self.write({'user_id': self.partner_id.user_id})
            
    def write(self, vals):
        res = super(Lead, self).write(vals)
        if 'stage_id' in vals:
            self.filtered(lambda m: m.stage_id.is_won and not m.closing_date).write({'closing_date': fields.Datetime.now()})
            self.filtered(lambda m: not m.stage_id.is_won).write({'closing_date': False})
