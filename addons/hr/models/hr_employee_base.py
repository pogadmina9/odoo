...
class HrEmployeeBase(models.AbstractModel):
    ...
    parent_id = fields.Many2one('hr.employee', 'Upline', compute="_compute_parent_id", store=True, readonly=False,
        check_company=True)
    gm_id = fields.Many2one('hr.employee', string='GM', compute="_compute_gm_id", store=True, readonly=False)
    manager_id = fields.Many2one('hr.employee', string='Manager', compute="_compute_manager_id", store=True, readonly=False)
    ...
    def _compute_parent_id(self):
        for employee in self.filtered('department_id.manager_id'):
            if employee.user_id.employee_id.id == employee.department_id.manager_id.id:
    ...
    def _compute_manager_id(self):
        for employee in self.filtered('department_id.manager_id'):
            if employee.user_id.employee_id.id == employee.department_id.manager_id.id:
                employee.manager_id = employee.department_id.parent_id.manager_id
            
            else:
                employee.manager_id = employee.department_id.manager_id

    @api.depends('department_id')
    def _compute_gm_id(self):
        for employee in self.filtered('department_id.gm_id'):
            if employee.user_id.employee_id.id == employee.department_id.gm_id.id:
                employee.gm_id = False
            
            else:
                employee.gm_id = employee.department_id.gm_id
