...
class HrEmployeePrivate(models.Model):
    ...
    def write(self, vals):
        ...
        if 'job_id' in vals:
            self.update_manager_bod()
            self.update_supervisor()
        return res
        
    def update_manager_bod(self):
        for rec in self:
            search_group = rec.sudo().env['res.groups'].search([('name', 'ilike', 'BOD')])
            list_manager = rec.sudo().env['hr.employee'].search(['|', ('job_id.name', 'ilike', 'Manager'), '|', ('job_id.name', 'ilike', 'GM'), ('job_id.name', 'ilike', 'Chief')])
            manager = list_manager.mapped("id")
            search_manager = rec.sudo().env['res.users'].search([('employee_id', 'in', manager)])
            for group in search_group:
                    group.sudo().write({'users': False})
                    group.sudo().write({'users': search_manager})
                    
    def update_supervisor(self):
        for rec in self:
            search_group = rec.sudo().env['res.groups'].search([('name', 'ilike', 'Supervisor')])
            list_spv = rec.sudo().env['hr.employee'].search(['|', ('job_id.name', 'ilike', 'Supervisor'), ('job_id.name', 'ilike', 'SPV')])
            spv = list_spv.mapped("id")
            search_spv = rec.sudo().env['res.users'].search([('employee_id', 'in', spv)])
            for group in search_group:
                    group.sudo().write({'users': False})
                    group.sudo().write({'users': search_spv})
